...
function reply(userText: string, model: string): string {
  ...
  if (/^(?:dame|give me|generate|make|create|hazme|genera)\b.*\b(?:word|pdf|excel|csv|document|documento)\b/i.test(t)) { const lang = /\bpdf\b/i.test(t) ? "document-pdf" : /\b(?:excel|csv)\b/i.test(t) ? "document-csv" : "document"; return `Here is the document.\n\n\`\`\`${lang}\n# Lab summary\n\nPrepared by the Helixona Assistant for review.\n\n## Results\n\n| Test | Result | Range |\n| --- | --- | --- |\n| Hemoglobin | 13.2 g/dL | 12.0–16.0 |\n| WBC | 11.8 | 4.0–11.0 |\n\n## Interpretation\n\n- Mild leukocytosis; repeat in 2 weeks.\n- Iron studies pending.\n\`\`\`\n\n*Sample response (${label}). Review before sending.*`; }
