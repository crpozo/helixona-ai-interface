...
export async function createDeps(env: NodeJS.ProcessEnv = process.env): Promise<Deps> {
  ...
  const repos = config.STORE_MODE === "memory"
    ? memoryRepos()
    : dynamoRepos(config.AWS_REGION!, { conversations: config.TABLE_CONVERSATIONS!, messages: config.TABLE_MESSAGES!, sessions: config.TABLE_SESSIONS!, audit: config.TABLE_AUDIT!, usage: config.TABLE_USAGE! });
